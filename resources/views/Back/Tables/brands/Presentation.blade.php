...
<div class="content">
    ...
    <table class="table table-bordered table-hover table-responsive content">
        ...
        <tbody>
            @if (count($brands)>0)
            @foreach ($brands as $item_customers)
            <tr class="hed">
                ...
                <td>
                    ...
                    <button type="button" idt="{{ $item_customers->id}}" idm="{{ $item_customers->pic}}"
                        route="{{ route('brands.destroy.dell', ['id'=>$item_customers->id,'img'=>$item_customers->pic]) }}"
    ...
    <br>
    <a style="background: #011a25;" href="{{ route('brands.create') }}" class="btn btn-primary btn-block btn-large">
