...
<div class="content">
    ...
    <table class="table table-bordered table-hover table-responsive content">
        ...
        <tbody>
            @if (count($services)>0)
            @foreach ($services as $item_customers)
            <tr class="hed">
                ...
                <td>{{ $item_customers->Subtitle}}</td>
                <td>{!! set_lab_lang($item_customers->Language) !!}</td>            
                <td>
                    <a rel="tooltip" href="{{ route('services.edit', ['id'=>$item_customers->id]) }}"
                    ...
                    <button type="button" idt="{{ $item_customers->id}}" idm="{{ $item_customers->Image}}"
                        route="{{ route('services.destroy.dell', ['id'=>$item_customers->id,'img'=>$item_customers->Image]) }}"
    ...
    <br>
    <a style="background: #011a25;" href="{{ route('services.create') }}" class="btn btn-primary btn-block btn-large">
