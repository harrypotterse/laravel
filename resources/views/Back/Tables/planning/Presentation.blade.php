...
<div class="content">
    ...
    <table class="table table-bordered table-hover table-responsive content">
        ...
        <tbody>
            @if (count($planning)>0)
            @foreach ($planning as $item_customers)
            <tr class="hed">
                <td><span class="badge">{{ $item_customers->id}}</span></td>
             
                <td>{{ $item_customers->Title}}</td>
                <td>{{ $item_customers->Subject}}</td>
                <td>{!! set_lab_lang($item_customers->Language) !!}</td>
                <td>
                    <a rel="tooltip" href="{{ route('planning.edit', ['id'=>$item_customers->id]) }}"
                    ...
                    <button type="button" idt="{{ $item_customers->id}}" idm="{{ $item_customers->Image}}"
                        route="{{ route('planning.destroy.dell', ['id'=>$item_customers->id,'img'=>$item_customers->Image]) }}"
    ...
    <br>
    <a style="background: #011a25;" href="{{ route('planning.create') }}" class="btn btn-primary btn-block btn-large">
